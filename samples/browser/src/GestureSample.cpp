...
void GestureSample::render(float elapsedTime)
{
    ...
    if (isGestureSupported(Gesture::GESTURE_LONG_TAP))
    {
        _font->drawText(L"Long tap supported", x, y, fontColor, _font->getSize());
        y += _font->getSize();
    }
    if (isGestureSupported(Gesture::GESTURE_DRAG))
    {
        _font->drawText(L"Drag supported", x, y, fontColor, _font->getSize());
        y += _font->getSize();
    }
    if (isGestureSupported(Gesture::GESTURE_DROP))
    {
        _font->drawText(L"Drop supported", x, y, fontColor, _font->getSize());
        y += _font->getSize();
    }
