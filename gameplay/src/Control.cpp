/*
    ...
    Changes to orginal document were done in lines: 12-13, 422-1731.
*/
...
void Control::notifyListeners(Control::Listener::EventType eventType)
{
    ...
    // Make sure we don't call this method from destructor.
    if (getRefCount() > 0)
        this->addRef();
...
void Control::updateBounds()
{
    ...
    if (_alignment != Control::ALIGN_TOP_LEFT || _isAlignmentSet)
    {
        ...
        {
            _bounds.y = ceilf(clipHeight * 0.5f - _bounds.height * 0.5f + (margin.top - margin.bottom) * 0.5f);
        }
        ...
        {
            _bounds.x = ceilf(clipWidth * 0.5f - _bounds.width * 0.5f + (margin.left - margin.right) * 0.5f);
        }
