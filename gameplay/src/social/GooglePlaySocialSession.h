 ...
 *
 * Note: ensure social.config has the following properties
 *
  @verbatim
    social
    {
...
public:
     ...
     */
    bool supportsChallenges() const { return false; }
    ...
    // methods called from JNI methods
    static void finishedSignIn();
    static void finishedAddingFriends();
    static void finishedAddingScores();
    static void finishedAddingAchievements();
    static void finishedLoadingData(const char *data);
    static void handleFriendsError(int errorCode);
    static void handleScoresError(int errorCode);
    static void handleAchievementError(int errorCode);
    static void handleLoadDataError(int errorCode);
    static void addFriend(const char *playerName);
    static void addScore(int rank, const char *playerName, int score);
    static void addAchievement(const char *name, const char *title, int value, int total, const char *date);
    static void setUser(const char *playerName, const char *playerId);
...
private:
    ...
    void getJNI(JNIEnv *&env, jclass &javaClass) const;
    void detachJNI() const;
    const SocialAchievement* getAchievement(const char* achievementId) const;
    const char* getMapping(const char *lookupId, const char *mapping) const;
