...
bool ScoreloopSocialSession::handleEvent(PlatformEvent *event)
{
	bps_event_t* bps_event = event;
    ...
    switch (result)
    {
		...
		default:
			if (result == SCUI_RESULT_CANCEL)
				session->getListener()->uiEvent(SocialSessionListener::ERROR_CANCEL);
			break;
    }
}
...
const char* ScoreloopSocialSession::getMapping(const char* identifier, const char *mapping) const
{
...
void ScoreloopSocialSession::submitAchievement(const char* achievementId, unsigned int value, bool isAchieved)
{
    ...
    if (isAchieved)
    {
    	rc = SC_LocalAchievementsController_SetAchievedValueForAwardIdentifier(_localAchievementsController, realAchievementId, &achieved);
    }
    else
    {
		rc = SC_LocalAchievementsController_SetValueForAwardIdentifier(_localAchievementsController, realAchievementId, value, &achieved);
    }
    ...
    if (achieved == SC_TRUE)
    {
    	const SocialAchievement *achievement = getAchievement(realAchievementId);
    	if (achievement)
...
void ScoreloopSocialSession::incrementAchievement(const char* achievementId, unsigned int increment)
{
	...
	rc = SC_LocalAchievementsController_IncrementValueForAwardIdentifier(_localAchievementsController, realAchievementId, &achieved);
...
SocialChallenge &ScoreloopSocialSession::addChallenge(SC_Challenge_h scoreloopChallenge)
{
	...
	SC_Score_h score = SC_Challenge_GetContenderScore((SC_Challenge_h)challenge.handle);
	challenge.opponentScore = (score) ? (float)SC_Score_GetResult(score) : 0;

	score = SC_Challenge_GetContestantScore((SC_Challenge_h)challenge.handle);
...
void ScoreloopSocialSession::displayChallenges() const
{
    ...
    if (rc != SC_OK)
    {
        print("Can't set challenge flags\n");
        return;
    }
   ...
   // rc = SCUI_Client_ShowChallengeCreateView(_uiClient);
    if (rc != SC_OK)
    {
        print("Failed to start a Challenges card! %s\n", SC_MapErrorToStr(rc));
    }
}
