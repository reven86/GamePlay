...
void AIState::enter(AIStateMachine* stateMachine)
{
    ...
    if (node)
        node->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(Node, stateEnter), static_cast<void*>(node), this);
}
...
void AIState::exit(AIStateMachine* stateMachine)
{
    ...
    Node* node = stateMachine->_agent->_node;
    if (node)
        node->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(Node, stateExit), static_cast<void*>(node), this);
}
...
void AIState::update(AIStateMachine* stateMachine, float elapsedTime)
{
    ...
    Node* node = stateMachine->_agent->_node;
    if (node)
        node->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(Node, stateUpdate), static_cast<void*>(node), this, elapsedTime);
}
