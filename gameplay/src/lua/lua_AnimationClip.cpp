...
static int lua_AnimationClip_setElapsedTime(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
    ...
    switch (paramCount)
    {
        case 2:
        {
                ...
                lua_type(state, 2) == LUA_TNUMBER)
            {
                ...
                AnimationClip* instance = getInstance(state);
                instance->setElapsedTime(param1);
                
                return 0;
            }

            lua_pushstring(state, "lua_AnimationClip_setElapsedTime - Failed to match the given parameters to a valid function signature.");
            lua_error(state);
            break;
        }
        default:
...
void luaRegister_AnimationClip()
{
    const luaL_Reg lua_members[] = 
    {
        ...
        {"setElapsedTime", lua_AnimationClip_setElapsedTime},
