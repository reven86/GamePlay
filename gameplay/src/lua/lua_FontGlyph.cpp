...
static Font::Glyph* getInstance(lua_State* state)
{
    ...
    return (Font::Glyph*)((gameplay::ScriptUtil::LuaObject*)userdata)->instance;
}

static int lua_FontGlyph__gc(lua_State* state)
{
...
static int lua_FontGlyph_bearingX(lua_State* state)
{
    // Validate the number of parameters.
    if (lua_gettop(state) > 2)
    {
    ...
    if (lua_gettop(state) == 2)
    {
        ...
        instance->bearingX = param2;
        return 0;
    }
    else
    {
        int result = instance->bearingX;
...
static int lua_FontGlyph_code(lua_State* state)
{
    // Validate the number of parameters.
    if (lua_gettop(state) > 2)
    {
...
static int lua_FontGlyph_uvs(lua_State* state)
{
    // Validate the number of parameters.
    if (lua_gettop(state) > 2)
    {
...
static int lua_FontGlyph_width(lua_State* state)
{
    // Validate the number of parameters.
    if (lua_gettop(state) > 2)
    {
...
void luaRegister_FontGlyph()
{
    ...
    {
        {"advance", lua_FontGlyph_advance},
        {"bearingX", lua_FontGlyph_bearingX},
    ...
    gameplay::ScriptUtil::registerClass("FontGlyph", lua_members, lua_FontGlyph__init, lua_FontGlyph__gc, lua_statics, scopePath);
