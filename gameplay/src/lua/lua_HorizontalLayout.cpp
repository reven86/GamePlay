...
static int lua_HorizontalLayout__gc(lua_State* state)
{
...
static int lua_HorizontalLayout_addRef(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_getRefCount(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_getRightToLeft(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_getSpacing(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_getType(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_release(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_setRightToLeft(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
static int lua_HorizontalLayout_setSpacing(lua_State* state)
{
    // Get the number of parameters.
    int paramCount = lua_gettop(state);
...
// Provides support for conversion to all known relative types of HorizontalLayout
static void* __convertTo(void* ptr, const char* typeName)
{
    HorizontalLayout* ptrObject = reinterpret_cast<HorizontalLayout*>(ptr);
...
static int lua_HorizontalLayout_to(lua_State* state)
{
    // There should be only a single parameter (this instance)
    if (lua_gettop(state) != 2 || lua_type(state, 1) != LUA_TUSERDATA || lua_type(state, 2) != LUA_TSTRING)
    {
        lua_pushstring(state, "lua_HorizontalLayout_to - Invalid number of parameters (expected 2).");
        lua_error(state);
        return 0;
    }
...
void luaRegister_HorizontalLayout()
{
    ...
    luaGlobal_Register_Conversion_Function("HorizontalLayout", __convertTo);
}
