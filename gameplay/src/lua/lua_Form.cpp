...
void luaRegister_Form()
{
    const luaL_Reg lua_members[] = 
    {
        ...
        {"setDrawOntoScreen", lua_Form_setDrawOntoScreen},
    ...
    switch (paramCount)
    {
        case 2:
        {
                ...
                lua_type(state, 2) == LUA_TBOOLEAN)
            {
                ...
                Form* instance = getInstance(state);
                instance->setDrawOntoScreen(param1);
                
                return 0;
            }

            lua_pushstring(state, "lua_Form_setDrawOntoScreen - Failed to match the given parameters to a valid function signature.");
            lua_error(state);
            break;
        }
        default:
