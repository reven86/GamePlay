    ...
    switch (paramCount)
    {
        case 1:
        {
            if ((lua_type(state, 1) == LUA_TUSERDATA || lua_type(state, 1) == LUA_TTABLE || lua_type(state, 1) == LUA_TNIL))
            {
                ...
                FileSystem::registerPackage(param1);
                
                return 0;
            }

            lua_pushstring(state, "lua_FileSystem_static_registerPackage - Failed to match the given parameters to a valid function signature.");
            lua_error(state);
            break;
        }
        default:
