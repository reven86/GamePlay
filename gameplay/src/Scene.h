...
void Scene::visitNode(Node* node, T* instance, bool (T::*visitMethod)(Node*))
{
    ...
    const MeshSkin* skin = node->getDrawable() ? node->getDrawable()->getSkin() : nullptr;
    if (skin && skin->_rootNode)
    {
        visitNode(skin->_rootNode, instance, visitMethod);
    }
...
void Scene::visitNode(Node* node, T* instance, bool (T::*visitMethod)(Node*,C), C cookie)
{
    ...
    if (skin && skin->_rootNode)
    {
        visitNode(skin->_rootNode, instance, visitMethod, cookie);
    }
